...
@Component({
  selector: 'jhi-membership-category',
...
export class MembershipCategoryComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.membershipCategoryService
      ...
      .subscribe(
        (res: HttpResponse<IMembershipCategory[]>) => {
  ...
  trackId(index: number, item: IMembershipCategory): number {
    return item.id!;
  }

  delete(membershipCategory: IMembershipCategory): void {
    const modalRef = this.modalService.open(MembershipCategoryDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.membershipCategory = membershipCategory;
  ...
  protected onSuccess(data: IMembershipCategory[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/membership-category'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
