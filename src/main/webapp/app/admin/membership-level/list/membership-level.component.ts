...
export class MembershipLevelComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.membershipLevelService
      ...
      .subscribe(
        (res: HttpResponse<IMembershipLevel[]>) => {
  ...
  trackId(index: number, item: IMembershipLevel): number {
    return item.id!;
  }

  delete(membershipLevel: IMembershipLevel): void {
    const modalRef = this.modalService.open(MembershipLevelDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.membershipLevel = membershipLevel;
  ...
  protected onSuccess(data: IMembershipLevel[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/membership-level'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
