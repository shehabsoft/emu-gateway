...
export class MembershipTypeComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.membershipTypeService
      ...
      .subscribe(
        (res: HttpResponse<IMembershipType[]>) => {
  ...
  trackId(index: number, item: IMembershipType): number {
    return item.id!;
  }

  delete(membershipType: IMembershipType): void {
    const modalRef = this.modalService.open(MembershipTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IMembershipType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/membership-type'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
